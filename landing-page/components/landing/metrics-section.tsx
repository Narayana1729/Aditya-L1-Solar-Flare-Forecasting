...
const metrics = [
  { 
    ...
    suffix: "", 
    prefix: "",
    label: "Telemetry rows ingested",
    sublabel: "from SoLEXS + HEL1OS instruments",
  },
  { 
    value: 12, 
    suffix: ".4 min", 
    ...
    sublabel: "average flare forecast advantage",
  },
  { 
    value: 740, 
    suffix: " days", 
    prefix: "",
    label: "SoLEXS coverage",
    sublabel: "Feb 2024 – Jun 2026 · Aditya-L1",
  },
  { 
    value: 708, 
    suffix: " days", 
    prefix: "",
    label: "HEL1OS coverage",
    sublabel: "Oct 2023 – Jun 2026 · Aditya-L1",
  },
];
    ...
    const render = () => {
      ...
      for (let x = 0; x < width; x += gridSize) {
        for (let y = 0; y < height; y += gridSize) {
          ...
          ctx.fillStyle = isLight ? "rgba(15, 23, 42, 0.18)" : "rgba(255, 255, 255, 0.04)";
          ctx.fill();
        }
      }
      const pulseY = (time * 30) % height;
      const isLight = typeof document !== "undefined" && !document.documentElement.classList.contains("dark");
      ctx.strokeStyle = isLight ? "rgba(15, 23, 42, 0.16)" : "rgba(255, 255, 255, 0.03)";
      ctx.lineWidth = 1;
      ctx.beginPath();
      ctx.moveTo(0, pulseY);
...
export function MetricsSection() {
  ...
  return (
    <section id="metrics" ref={sectionRef} className="relative py-32 lg:py-40 overflow-hidden bg-background text-foreground">
      <GridBackground />
      ...
      <div className="relative z-10 max-w-[1400px] mx-auto px-6 lg:px-12">
          ...
          <div className="lg:col-span-8 lg:col-start-1">
            <div className="flex items-center gap-4 mb-6">
              ...
              <span className="text-sm font-mono text-muted-foreground">
                {time ? `${time.toLocaleTimeString("en-GB")} IST` : ""}
              </span>
            </div>
            ...
            <h2 className={`text-6xl md:text-7xl lg:text-[140px] font-display tracking-tight leading-[0.95] transition-all duration-1000 ${
              ...
              Solar flare
              <br />
              <span className="text-muted-foreground">model metrics.</span>
            </h2>
          </div>
        </div>

        {/* Organic graph image wrapped in a styled monitor frame */}
        <div className={`w-full mb-24 lg:mb-32 transition-all duration-1000 delay-200 ${
          isVisible ? "opacity-100" : "opacity-0"
        }`}>
          <div className="bg-black border border-white/10 rounded-2xl overflow-hidden p-2 lg:p-4 shadow-xl">
            <img
              src="https://hebbkx1anhila5yf.public.blob.vercel-storage.com/real-time-graph-INFmn3u0MlUwvNPynoIhwxtPaPjxM5.png"
              alt="Real-time Solar Telemetry Graph"
              className="w-full h-auto object-contain rounded-lg"
            />
          </div>
        </div>

        {/* Metrics grid — 5 columns total: first is a rectangle (2 cols), others are squares (1 col each) */}
        <div className="grid lg:grid-cols-5 gap-6">

          {/* Large metric (Telemetry rows ingested) — Col-span-2 (Rectangle) */}
          <div className={`lg:col-span-2 bg-foreground/[0.02] border border-foreground/10 p-10 lg:p-14 transition-all duration-700 ${
          ...
          {/* Other 3 metrics — Col-span-1 (Squares) */}
          {metrics.slice(1).map((metric, index) => (
            <div
              key={metric.label}
              className={`lg:col-span-1 bg-foreground/[0.02] border border-foreground/10 p-8 flex flex-col justify-between gap-6 transition-all duration-700 ${
                isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-12"
              }`}
            ...
            >
              ...
              <div className="text-3xl md:text-4xl lg:text-[40px] font-display tracking-tight w-full whitespace-nowrap">
        ...
        {/* Bottom ticker */}
        <div className={`mt-16 pt-8 border-t border-foreground/10 flex flex-wrap items-center justify-between gap-x-12 gap-y-4 text-sm font-mono text-muted-foreground transition-all duration-1000 delay-500 ${
          isVisible ? "opacity-100" : "opacity-0"
        }`}>
          <div className="flex flex-wrap gap-x-8 gap-y-2">
            ...
            <span className="text-foreground">TSS · HSS · PSS</span>
          </div>
        </div>
      </div>
    </section>
