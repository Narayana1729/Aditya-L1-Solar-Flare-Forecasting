...
def parse_single_solexs(f, cadence):
    try:
        path_lower = f.lower()
        ...
        with fits.open(f) as hdul:
            ...
            return df.set_index('timestamp').resample(cadence).mean()
    except Exception:
        pass
    return None

def load_solexs(cadence='1min'):
    """Find and load all SoLEXS .lc.gz files, resample, and return df in parallel."""
    ...
    print(f"Parsing {len(files)} SoLEXS files in parallel...")
    results = Parallel(n_jobs=-1, batch_size=50)(
        delayed(parse_single_solexs)(f, cadence) for f in files
    )
    dfs = [r for r in results if r is not None]
    if not dfs:
        return pd.DataFrame()
        
    print("Combining SoLEXS dataframes...")
    merged_df = pd.concat(dfs, axis=0)
    merged_df = merged_df.groupby(merged_df.index).mean()
    return merged_df

def parse_single_hel1os(f, cadence):
    try:
        ...
        with fits.open(f) as hdul:
            ...
            for hdu in hdul[1:]:
                ...
                file_dfs.append(df)
            if file_dfs:
                return pd.concat(file_dfs, axis=1)
    except Exception:
        pass
    return None

def load_hel1os(cadence='1min'):
    """Find and load all HEL1OS lightcurve_*.fits files, resample, and return df in parallel."""
    ...
    print(f"Parsing {len(files)} HEL1OS files in parallel...")
    results = Parallel(n_jobs=-1, batch_size=100)(
        delayed(parse_single_hel1os)(f, cadence) for f in files
    )
    dfs = [r for r in results if r is not None]
    if not dfs:
        return pd.DataFrame()
        
    print("Combining HEL1OS dataframes...")
    merged_df = pd.concat(dfs, axis=0)
    merged_df = merged_df.groupby(merged_df.index).mean()
    return merged_df
