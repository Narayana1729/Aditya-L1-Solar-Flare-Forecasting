...
def load_data_and_predict(input_path, model_path, scaler_path, lookback=30, forecast_horizon=30):
    """
    Load data, scale features, run LSTM predictions, and return inputs, targets, predictions, and metadata.
    """
    ...
    # Re-create is_flare and target columns exactly as in train_lstm.py
    ...
    df_meta = df_meta[~nan_mask].reset_index(drop=True)
...
def calculate_event_lead_times(timestamps, is_flare_seq, alert_triggers, forecast_horizon=30):
    ...
    """
    # Find start and end indices of flare events
    ...
    for event_id, group in event_groups:
        ...
        if not alerts_in_window.empty:
            ...
            lead_time = (start_time - first_alert_time).total_seconds() / 60.0  # in minutes
...
def evaluate_ensemble(lstm_probs, y_true, df_meta, w1=0.7, w2=0.3, threshold=0.5, forecast_horizon=30):
    """
    Combine LSTM and physics precursor score, evaluate and print metrics.
    """
...
def main():
    ...
    print(f"Saved validation predictions to data/processed/validation_predictions.csv")
    
    print("\n------------------- Evaluation Metrics -------------------")
    print(f"Accuracy Metric     | Value")
    print(f"----------------------------------------------------------")
    ...
    df_res = df_res.sort_values('f1_score', ascending=False)
    
    print("\nTop 5 Configuration Combinations (Sorted by F1-Score):")
    print(df_res[['weight_lstm', 'weight_physics', 'threshold', 'f1_score', 'far', 'event_recall', 'avg_lead_time_minutes']].head(5).to_string(index=False))
